    ...
    var admin = {
        ...
        table: {
            ...
            buildOperatHtml: function (operat) {
                ...
                } else if(operat.method === 'openew'){
                    formatOperat.method = formatOperat.method !== '' ? 'target="_blank" href="' + formatOperat.url + '" data-title="' + formatOperat.title + '" ' : '';
                } else {
                    ...
                    if (typeof item === 'string') {
                        switch (item) {
                            ...
                            case 'delete':
                                ...
                                //***自定义验证是否显示操作元素***
                                var showStatus = true;
                                if(operat.checkFiled != undefined && operat.checkCondition != undefined && operat.checkType != undefined)
                                {
                                    /**具体条件
                                     * 1: == 2: > 3: >= 4:< 5: <= 6: !=
                                     * */
                                    if(operat.checkType == 1)
                                    {
                                        if(!(data[operat.checkFiled] == operat.checkCondition))
                                        {
                                            showStatus = false;
                                        }
                                    }
                                    if(operat.checkType == 2)
                                    {
                                        if(!(data[operat.checkFiled] > operat.checkCondition))
                                        {
                                            showStatus = false;
                                ...
                                if (admin.checkAuth(operat.auth, elem) && showStatus == true) {
                                    html += admin.table.buildOperatHtml(operat);
                                }
                                break;
                        }
                        ...
                        $.each(item, function (i, operat) {
                            ...
                            operat.method = operat.method || 'open';
                            operat.field = operat.field || 'id';
                            if(operat.change_title != undefined) //让动态变量可以有操作
                            {
                                operat.title = data[operat.change_title];
                            } else {
                                operat.title = operat.title || operat.text;
                            }
                            operat.text = operat.text || operat.title;
